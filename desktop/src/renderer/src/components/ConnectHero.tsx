...
export function ConnectHero({
  ...
  const connectedTitle = subscription?.title?.trim() || activeProfile?.name || t("Connected")
  const lastError = status?.lastError
    ? status.lastError.includes('no active profile')
      ? t("Import or activate a profile first.")
      : status.lastError
    : null
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="mb-3 flex flex-wrap items-center gap-2.5">
              <p className="section-label">{t("Connection")}</p>
              <Badge variant={running ? 'success' : 'secondary'} className="px-2 py-0 text-[10px]">
                {connectionAction ?? (running ? t("Core running") : t("Disconnected"))}
              </Badge>
            </div>
            <h2
            ...
            >
              {running ? connectedTitle : t("Ready to connect")}
            </h2>
            <p className="mt-2 max-w-md text-sm text-muted-foreground">
              {running
                ? modeLabel === 'TUN' ? t("TUN is running. Routing follows the profile rules.") : t("Proxy is running. Use the system proxy or configure your apps to route traffic through it.")
                : hasProfile
                  ? t("Choose Proxy or TUN in Settings, then connect.")
                  : t("Add a subscription or profile file on Profiles, then connect.")}
            </p>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {activeProfile?.url && (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    title={t("Open the current subscription URL")}
                ...
                {subscription?.supportUrl && (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    title={t("Open subscription support")}
          ...
          {running && (
            <div className="shrink-0 space-y-1.5 text-right text-xs text-muted-foreground">
              <div>
                <span className="block text-[10px] uppercase tracking-wide">{t("Download")}</span>
                <span className="font-medium tabular-nums text-foreground">{formatRate(downRate)}</span>
              </div>
              <div>
                <span className="block text-[10px] uppercase tracking-wide">{t("Upload")}</span>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          {!hasProfile && onGoProfiles ? (
            ...
            <Button type="button" variant="destructive" className="min-w-[140px]" disabled={busy} onClick={onDisconnect}>
              ...
              {connectionAction ?? t("Disconnect")}
            </Button>
          )}
        </div>

        {lastError && <p className="mt-4 text-sm text-destructive">{t(lastError)}</p>}

        <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-border pt-4 text-xs text-muted-foreground">
          <span>
            {t("Mode:")}{' '}<span className="font-medium text-foreground">{modeLabel}</span>
          </span>
          {tun?.enabled && <><span>
            {t("VPN helper:")}{' '}{' '}
            <span className="text-foreground">
              {tun?.helperRunning ? t("running") : tun?.helperInstalled ? t("installed") : t("not installed")}
            </span>
          </span>
          <span>
            {t("Setup:")}{' '}{' '}
