...
/* Build version. When we switch to building Go from source, a git hash will
 * be substituted here via CMake configure_file. Currently a placeholder. */
#ifndef GIT_VERSION
...
JNIEXPORT void JNICALL
Java_com_github_kr328_clash_core_bridge_Bridge_nativeHealthCheckGroup(JNIEnv *env, jobject thiz,
                                                                      jobject completable,
                                                                      jstring name) {
    TRACE_METHOD();
    jobject _completable = new_global(completable);
    scoped_string _name = get_string(name);
    healthCheckGroup(_completable, _name);
}
...
Java_com_github_kr328_clash_core_bridge_Bridge_nativeHealthCheckProxy(JNIEnv *env, jobject thiz,
    ...
    healthCheckProxy(_completable, _group, _proxy);
}
...
JNIEXPORT jstring JNICALL
Java_com_github_kr328_clash_core_bridge_Bridge_nativeMihomoVersion(JNIEnv *env, jobject thiz) {
    TRACE_METHOD();
    /* getMihomoVersion returns a C-string allocated by the Go runtime —
     * we free it via scoped_string like any other //export strings. */
    scoped_string version = getMihomoVersion();
    return new_string(version);
}

/* ----- Callbacks from Go (libclash.so) to JVM. ----- */
...
JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    m_logcat_interface_received = find_method(c_logcat_interface, "received",
                                              "(Ljava/lang/String;)V");
    m_clash_exception = find_method(_c_clash_exception, "<init>", "(Ljava/lang/String;)V");
    m_get_message = find_method(c_throwable, "getMessage", "()Ljava/lang/String;");
    m_open = (*env)->GetStaticMethodID(env, _c_content, "open", "(Ljava/lang/String;)I");
